				      ...
				      struct fib6_table *table)
{
	...
	if (rt) {
		...
		rt6shi = kmalloc(sizeof(*rt6shi), GFP_ATOMIC);
		if (rt6shi) {
			atomic_set(&rt6shi->ref, 1);
			rt6shi->expires = 0;
			rt->rt6shi = rt6shi;
		} else {
...
static void ip6_dst_destroy(struct dst_entry *dst)
{
	...
	if (rt->rt6shi && atomic_dec_and_test(&rt->rt6shi->ref))
		kfree(rt->rt6shi);
}
...
static bool rt6_check_expired(const struct rt6_info *rt)
{
	if (rt->rt6i_flags & RTF_EXPIRES) {
		...
		unsigned long parent_expires = rt->rt6shi->expires;
...
static int ip6_rt_reins(struct rt6_info *rt)
{
	struct fib6_table *table;
	struct rt6_info *nrt;
	struct net *net;
	int err = 0;
	if (rt->rt6i_node == NULL)
		/* someone did it */
		return 0;
			     ...
			     const struct in6_addr *dest)
{
	...
	atomic_inc(&ort->rt6shi->ref);
	rt->rt6shi = ort->rt6shi;
}
